...
@petercord.on_cmd("sticker", about={
    ...
    'petercord': "{tr}sticker [text | reply to message]\n"
...
async def anime_sticker(message: Message):
    ...
    if '-ggl' in message.flags:
        try:
            stickers = await petercord.get_inline_bot_results(
                "stickerizerbot",
                f"#12{demojify(text)}")
    ...
    try:
        ...
        saved = await petercord.send_inline_bot_result(
            chat_id="me",
            query_id=stickers.query_id,
            result_id=stickers.results[0].id,
            hide_via=True
        )
        saved = await petercord.get_messages("me", int(saved.updates[1].message.id))
        message_id = replied.message_id if replied else None
        await petercord.send_sticker(
            chat_id=message.chat.id,
