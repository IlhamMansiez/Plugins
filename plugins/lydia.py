...
@petercord.on_cmd("lydia", about={
    ...
    'petercord': "{tr}lydia [flag] [reply to user]"})
    ...
    # Group Features Won't be displayed in Help Info For Now 😉
        ...
        for chat_id in ACTIVE_CHATS:
            if not str(chat_id).startswith("-100"):
                continue
            chat_ = await petercord.get_chat(chat_id)
            title = chat_.title
            msg += f"{title} {chat_id}\n"
        ...
        for user_id in ACTIVE_CHATS:
            if str(user_id).startswith("-100"):
                continue
            try:
                u_info = await petercord.get_user_dict(user_id)
...
@petercord.on_filters(~filters.me & ~filters.edited & (filters.mentioned | filters.private), group=2)
async def lydia_ai_chat(message: Message):
    ...
    if data:
        ...
        try:
            out = ''
            await petercord.send_read_acknowledge(
                chat_id=chat_id,
                message=message,
                clear_mentions=True)
