...
@petercord.on_cmd("vote", about={
    'header': "Vote poll",
    'description': "Options Should be in numeric",
    'petercord': "{tr}vote [option | reply to poll]",
...
async def vote_poll(msg: Message):
    ...
    if replied and replied.poll:
        ...
        try:
            await petercord.vote_poll(msg.chat.id, replied.message_id, option)
...
@petercord.on_cmd("stop", about={
    'header': "Stop a poll which was sent by you.",
    'petercord': "{tr}stop [reply to poll]"},
    allow_private=False)
async def stop_poll(msg: Message):
    ...
    if replied and replied.poll:
        try:
            await petercord.stop_poll(msg.chat.id, replied.message_id)
        except Exception as e_f:
            await msg.err(e_f)
        else:
...
@petercord.on_cmd("retract", about={
    'header': "Retract your vote in a Poll",
    'petercord': "{tr}retract [reply to poll]"},
    allow_private=False)
async def retract_vote(msg: Message):
    ...
    if replied and replied.poll:
        try:
            await petercord.retract_vote(msg.chat.id, replied.message_id)
        except Exception as e_f:
            await msg.err(e_f)
        else:
