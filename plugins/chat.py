...
async def join_chat(message: Message):
    ...
    try:
        ...
        await petercord.join_chat(text)
        await petercord.send_message(text, f"```Joined {chat.title} Successfully...```")
...
async def leave_chat(message: Message):
    ...
    try:
        await petercord.send_message(text, "```Good bye, Cruel World... :-) ```")
        await petercord.leave_chat(text)
    except UsernameNotOccupied:
    ...
    if not user_id:
        try:
            ...
            if chat.type in ['group', 'supergroup']:
                link = await petercord.export_chat_invite_link(chat_id)
                await message.edit(
...
@petercord.on_cmd("tagall", about={
    'header': "Tagall recent 100 members with caption",
    'petercord': "{tr}tagall [Text | reply to text Msg]"},
...
@petercord.on_cmd("stagall", about={
    'header': "Silent tag recent 100 members with caption",
    'petercord': "{tr}stagall [Text | reply to text Msg]"},
...
async def stagall_(message: Message):
    ...
    chat_id = message.chat.id
    chat = await petercord.get_chat(chat_id)
    ...
    message_id = replied.message_id if replied else None
    member = petercord.iter_chat_members(chat_id)
...
@petercord.on_cmd("tadmins", about={
    'header': "Tag admins in group",
    'petercord': "{tr}tadmins [Text | reply to text Msg]"},
    allow_private=False)
...
async def set_chat(message: Message):
    ...
    if not message.flags:
        ...
        return
    chat = await petercord.get_chat(message.chat.id)
    if '-ddes' in message.flags:
        if not chat.description:
            await message.edit(
                "```Chat already not have description...```", del_in=5)
        else:
            await petercord.set_chat_description(message.chat.id, "")
    ...
    if '-title' in message.flags:
        await petercord.set_chat_title(message.chat.id, args.strip())
        await message.edit("```Chat Title is Successfully Updated...```", del_in=3)
    elif '-uname' in message.flags:
        try:
            await petercord.update_chat_username(message.chat.id, args.strip())
        except ValueError:
        ...
        try:
            await petercord.set_chat_description(message.chat.id, args.strip())
        except BadRequest:
            await message.edit(
